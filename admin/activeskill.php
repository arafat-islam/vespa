...
$select_active_skill_query = "SELECT count(*) as active_skills FROM skill WHERE status = 1";

if($post = $db->select($select_active_skill_query)) {
    $result = $post->fetch_assoc();

    if ($result['active_skills'] >= 3) {
        $_SESSION['limit'] = "You cannot add more than 3 Items";
        header("location: skill.php");
    } else {
        $active = "UPDATE skill SET status = 1 WHERE id = $id";
